...
const WelcomeScreen = ({ isLoadingImage }: { isLoadingImage: boolean }) => (
  <div className={cn(
      ...
      <ImageIcon 
          className={cn(
              "w-12 h-12 text-primary/30 mb-4 transition-all duration-500 ease-in-out",
              isLoadingImage && "text-primary scale-110 [filter:drop-shadow(0_0_10px_hsl(var(--primary)))] animate-pulse"
          )}
          style={{ animationDuration: '1.5s' }}
      />
      <h3 className={cn(
          "font-semibold text-lg text-foreground/70 transition-colors duration-500",
...
const Whiteboard: React.FC<WhiteboardProps> = ({ state, isLoading }) => {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-black p-4">
        <div key={state.imageUrl} className="relative w-full h-full animate-in fade-in duration-500">
            <Image 
                src={state.imageUrl}
