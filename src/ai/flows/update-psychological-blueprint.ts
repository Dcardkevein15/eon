  ...
  async ({ userId, fullChatHistory }) => {
    ...
    try {
        ...
        if (!newBlueprint) {
          ...
          // Don't throw here, just log and exit.
          return;
        }
        ...
        if (error.code === 'permission-denied') {
            const context: SecurityRuleContext = {
                ...
                operation: 'write', // getDoc is read, but setDoc is write. The error is likely on write.
                requestResourceData: {
                    blueprint: 'Generated by AI', // Don't log the full blueprint
        ...
        // Even if we emit a custom error, we should still re-throw the original error
        // to let the caller know something went wrong. The custom error is for display.
        throw error;
    }
  }
);
