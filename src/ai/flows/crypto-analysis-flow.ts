...
// Esta función ahora es una utilidad interna, no una herramienta de IA.
...
// --- Prompts de los Agentes (simplificados, sin herramientas) ---
...
export async function runCryptoAnalysis(input: z.infer<typeof CryptoAnalysisInputSchema>): Promise<FullCryptoAnalysis> {
      ...
      // 1. Obtener datos de mercado directamente
      ...
      // 4. Invocar analistas con datos pre-cargados
      const { output: apexOutput } = await apexPrompt({ 
          analystName: 'Apex' as const, 
          cryptoName,
          technicalSummary,
          // El resto de campos no son necesarios para este prompt simplificado
          days: input.days, debateHistory: '', identityDescription: '', previousAlphaState: ''
      });
      if (!apexOutput?.argument) throw new Error("Apex no pudo generar un análisis.");
      
      const { output: heliosOutput } = await heliosPrompt({ 
          analystName: 'Helios' as const, 
          cryptoName,
          technicalSummary, // Se pasa pero el prompt lo ignora
          days: input.days, debateHistory: '', identityDescription: '', previousAlphaState: ''
      });
      ...
      const finalResult: FullCryptoAnalysis = {
        ...
        signals: synthesizerResult.signals.map(s => ({
            ...s,
            // Asegurar que el precio tenga un valor numérico
